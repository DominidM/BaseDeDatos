...
namespace T3_BD_CSharp
{
    public partial class PacienteForm : Form
    {
        ...
        {
            string query = "SELECT * FROM Paciente"; // Limitar a los primeros 10 registros
        ...
        public DataTable ObtenerDatosDeBaseDeDatos10(string filtro = "")
        {
            string query = "SELECT TOP 10 * FROM Paciente"; // Aquí puedes modificar la consulta
            if (!string.IsNullOrEmpty(filtro))
            {
                query += " WHERE Nombre LIKE @Filtro OR ApellidoPaterno LIKE @Filtro OR ApellidoMaterno LIKE @Filtro"; // Cambia "Nombre" y "Apellido" por las columnas que quieras filtrar
            }
        ...
        // Evento de cambio de texto en el TextBox de búsqueda
        private void txtBuscar_TextChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                datosFiltrados = ObtenerDatosDeBaseDeDatos10(textoBusqueda); // Llamamos al método con filtro
            }
        ...
        private DataTable CargarDatosDesdeBD(int limite, SqlConnection conn)
        {
            ...
            try
            {
                string query = limite > 0
                    ? "SELECT TOP (@Limite) * FROM Paciente"
        ...
        private void EliminarFilaDeBD(int rowIndex, ConexionBD conexionBD)
        {
            try
            {
                ...
                string query = "DELETE FROM Paciente WHERE Codigo = @Codigo"; // Cambia "Paciente" y "ID" según tu tabla
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
        ...
        public void InsertarDatos()
        {
            ...
            string apellidopaterno = textbox3.Text;
            string apellidomaterno = textbox4.Text;
            DateTime fechanacimiento = dateTimePicker1.Value;
            string genero = combobox.Text;
            string ciudad = textbox5.Text;
            string provincia = textBox8.Text;
            string telefonoText = textBox9.Text;
            string correo = textBox10.Text;
            string dniText = textBox11.Text;
            string tiposangre = comboBox1.Text;
            string alergia = textBox12.Text;
            ...
            // Validar que los campos de teléfono y DNI sean numéricos
            if (!int.TryParse(telefonoText, out int telefono))
            {
            ...
            string query = "INSERT INTO Paciente (Nombre, ApellidoPaterno, ApellidoMaterno, FechaNacimiento, Genero, Ciudad, Provincia, Telefono, Correo, DNI, TipoSangre, Alergia) " +
                           "VALUES (@Nombre, @ApellidoPaterno, @ApellidoMaterno, @FechaNacimiento, @Genero, @Ciudad, @Provincia, @Telefono, @Correo, @DNI, @TipoSangre, @Alergia)";
